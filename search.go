...
func (l *Conn) Search(searchRequest *SearchRequest) (*SearchResult, error) {
	...
	result, err := l.fetchSearchResult(msgCtx)
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (l *Conn) issueSearchRequest(searchRequest *SearchRequest) (*messageContext, error) {
...
func (l *Conn) fetchSearchResult(msgCtx *messageContext) (*SearchResult, error) {
	result := &SearchResult{
				...
				if oid == IntermediateResponseSyncInfo {
					...
					if siv.Tag == 3 { // For now we only handle syncIdSet
						...
						for _, s := range siv.Children[2].Children { // SET OF syncUUID
							...
							//entry.Attributes = append(entry.Attributes, attr)
							syncState := &ControlSyncState{State:uint32(state), EntryUUID:s.ByteValue}
